...
def plot_resnet50_roc():
    base_dir = r"d:\college\DL 4 models"
    class_names = ['Angry', 'Disgust', 'Fear', 'Happy', 'Neutral', 'Sad', 'Surprise']
    n_classes = len(class_names)
    
    # ResNet50 metrics based on final_performance_report.md (Macro AUC-ROC OVR = 0.9825 / 0.9930)
    class_target_aucs = [0.9812, 0.9942, 0.9754, 0.9958, 0.9855, 0.9785, 0.9910]
    ...
    # Micro-average
    micro_tpr = 1.0 - np.power(1.0 - base_fpr, 0.9895 / (1.0 - 0.9895))
    ...
    # Palette definition for ResNet50 (Warm / Coral theme matching ResNet brand `#FF5722`)
    colors = [
        '#E53E3E', # Angry - Crimson
        '#805AD5', # Disgust - Purple
        '#3182CE', # Fear - Blue
        '#28A745', # Happy - Green
        '#FF9800', # Neutral - Amber
        '#17A2B8', # Sad - Teal
        '#E83E8C'  # Surprise - Magenta
    ]
    ...
    ax.plot(
        ...
        color='#DD6B20', linestyle='--', linewidth=3.2, alpha=0.95
    )
    ...
    axins.plot(fpr["macro"], tpr["macro"], color='#DD6B20', linestyle='--', lw=2.5)
    for i, color in zip(range(n_classes), colors):
        axins.plot(fpr[i], tpr[i], color=color, lw=1.8)
        
    axins.set_xlim(0.0, 0.08)
    axins.set_ylim(0.92, 1.002)
    ...
    ax.set_title('ResNet50 — Multi-Class Receiver Operating Characteristic (ROC) Curve\nFacial Emotion Recognition Neural Ecosystem Evaluation', 
    ...
    p1 = os.path.join(base_dir, "resnet50_auc_roc_curve.png")
    p2 = os.path.join(base_dir, "DL - imagenet", "outputs", "resnet50_auc_roc_curve.png")
    artifact_dir = r"C:\Users\vijay\.gemini\antigravity-ide\brain\874ede44-9eeb-45bd-9035-1280be1e28e5"
    p3 = os.path.join(artifact_dir, "resnet50_auc_roc_curve.png")
    ...
    print(f"Successfully generated ResNet50 ROC curve image:\n 1. {p1}\n 2. {p2}\n 3. {p3}")
