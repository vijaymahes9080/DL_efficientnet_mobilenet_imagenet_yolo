...
def test_model(name, folder, script_path, model_type='tflite', benchmark=False):
    ...
    try:
        ...
        if model_type == 'pt':
            ...
            # YOLO predict in chunks to ensure batch processing and stability
            batch_size = 32
            for i in range(0, len(images), batch_size):
                chunk = [img.astype(np.uint8) for img in images[i:i+batch_size]]
                results = model.predict(chunk, verbose=False)
                for r in results:
