...
3.  **Upload Pre-Structured Folders**: Drag and drop the following folders from `d:\current project\DL -YOLO` into `DL/`:
...
| 4 | `04_model_architectures_comparison.ipynb` | YOLOv8-Nano vs Small vs Medium benchmarking. |
...
*   **XAI Heatmaps**: If Grad-CAM heatmaps are blurry, try adjusting the `target_layer_idx` in Notebook 08 (e.g., `-2` for YOLOv8 classification backbones).
